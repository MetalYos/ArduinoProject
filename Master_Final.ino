...
void loop() { 
  ...
  if (execCommand)
  {
    if (digitalRead(hall) == LOW)
    {
      ...
      // Check weight if it is one of the large containers
      if (container_int == 1 || container_int == 2)
      {
        // Check if bowl is on the scale in case the user removed it during travel
        ...
        {
          // If the bowl never filled with the wanted weight, go back
          goBack();
